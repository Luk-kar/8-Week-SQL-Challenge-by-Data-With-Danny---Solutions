...
WITH months_by_payment AS (
WITH RECURSIVE monthly_plans_by_month AS (
  SELECT
    customer_id,
    start_date,
    CASE
        ...
        THEN
  		COALESCE(
          next_start_date - INTERVAL '1 month',
  ...
  SELECT 
    ...
    plan_id
  FROM 
    monthly_plans_by_month
  WHERE
    start_date::DATE <= end_day
      AND NOT start_date::DATE >= end_day
)
 SELECT 
  ...
  WHERE
    ...
    SELECT
        ...
        CASE
            ...
            OR (
                LAG(m.plan_id) OVER (PARTITION BY m.customer_id ORDER BY m.start_date) IN (1, 2)
                AND m.plan_id = 3
            )
            THEN
...
-- Insert the results into the payments table
